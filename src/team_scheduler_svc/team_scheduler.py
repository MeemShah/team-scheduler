...
from ..exceptions import (
    WeekendException,
    InitialDateAfterQueryDateError,
    NotFoundError,
    InternalServerError,
    EmptyTeamListError,
)
...
class TeamScheduler:
    ...
    def _calculate_total_working_days_count(
        self, initial_date: date, query_date: date, working_days: List[str]
    ) -> int:
        ...
        # Count working days for the first week
        ...
        # Count working days for the last week
        ...
        number_of_working_days_in_week = len(working_days)
        remaining_weeks = day_diff // 7
        total_working_days += remaining_weeks * number_of_working_days_in_week
    ...
    def _determine_working_pair(
        self, total_working_days: int, team_pairs: List[Tuple[str, str]]
    ) -> Tuple[str, str]:
    ...
    def get_schedule(self, team_id: int, query_date: date):
        try:
            ...
            total_working_days = self._calculate_total_working_days_count(
                team_info.initial_start_date, query_date, team_info.working_days
            )
            team = self._determine_working_pair(total_working_days, team_info.team_pairs)

            return team, total_working_days

        except (EmptyTeamListError, WeekendException, InitialDateAfterQueryDateError, NotFoundError):
            raise
    ...
    def add_team_pair(self, member_1: str, member_2: str, team_id: int):
        try:
            _ = self.team_member_repo.add_pair(
                AddPairRequest(member_1=member_1, member_2=member_2, team_id=team_id)
            )
            return None
        except NotFoundError:
            raise
        ...
        try:
            ...
            for i in range(7):
                ...
                if day_name in team_info.working_days:
                    total_working_days = self._calculate_total_working_days_count(
                        team_info.initial_start_date, day_date, team_info.working_days
                    )
                    working_pair = self._determine_working_pair(total_working_days, team_info.team_pairs)
                    schedule.append({"date": day_date.isoformat(), "day": day_name, "pair": working_pair})
                else:
                    schedule.append({"date": day_date.isoformat(), "day": day_name, "pair": "Weekend"})

            return schedule
        except (EmptyTeamListError, NotFoundError):
            raise
        except Exception as e:
